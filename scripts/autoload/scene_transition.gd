...
func start_transition(fade_in_duration: float, wait_duration: float, fade_out_duration: float) -> void:
	...
	transition_tween.tween_property(fade_rect, "color", Color.BLACK, fade_in_duration)
	transition_tween.tween_callback(
		func():
			waiting = true
	...
	transition_tween.tween_property(fade_rect, "color", Color(0.0, 0.0, 0.0, 0.0), fade_out_duration)
	transition_tween.tween_callback(
		func():
			in_transition = false
